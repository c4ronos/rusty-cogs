...
class WaifuIM(commands.Cog):
    ...
    __author__ = ["Shit Show Development", "rusty-man"]
    ...
    async def help(self, ctx):
        ...
        tags = f'{versatile_tags}, {nsfw_tags}' if ctx.channel.is_nsfw() else versatile_tags
    ...
    async def random(self, ctx):
        ...
        async with self.session.get(search_endpoint, params=params) as response:
            ...
            upload_date = datetime.fromisoformat(uploaded_at).date().strftime("%B %d, %Y")
    ...
    async def tag(self, ctx, tag):
        ...
        params = {'included_tags': tag, 'is_nsfw': 'true'} if ctx.channel.is_nsfw() else {'included_tags': tag}
        ...
        if tag in versatile_tags or tag in nsfw_tags:
            await send_image()
        else:
